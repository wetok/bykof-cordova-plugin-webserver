@objc(Webserver) class Webserver : CDVPlugin {
    ...
    func fileRequest(request: GCDWebServerRequest, path: String) -> GCDWebServerResponse {
        ...
        var pathWithRootPath = path;
        if (self.rootPath != ""){
            var sep = "";
            if (!path.hasPrefix("/")){
                sep = "/";
            }
            pathWithRootPath = self.rootPath + sep + path;
        }
        
        // Check if file exists, given its path
        if !(FileManager.default.fileExists(atPath: pathWithRootPath)) {
            return GCDWebServerResponse(statusCode: 404);
        }

        if (request.hasByteRange()) {
            return GCDWebServerFileResponse(file: pathWithRootPath, byteRange: request.byteRange)!
        }

        return GCDWebServerFileResponse(file: pathWithRootPath)!
    }
    ...
    func start(_ command: CDVInvokedUrlCommand) {
        ...
        var rootPath = ""
        let rootPathArgument = command.argument(at: 1)
        if rootPathArgument != nil {
            rootPath = rootPathArgument as! String
